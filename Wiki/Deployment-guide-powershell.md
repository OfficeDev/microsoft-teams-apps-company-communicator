...
- Fill-in the Deployment\parameters.json file with required parameters values for the script. Replace << value >>; with the correct value for each parameter.
  ...
  ![Powershell deployment guide](images/param_file_view.png)
    ...
    - **SubscriptionTenantId** - Id of the tenant to deploy to (If you are not sure how to get Tenant ID, please check Azure Active Directory in Azure Portal. Under Manage, click Properties. The tenant ID is shown in the Directory ID box). e.g 98f3ece2-3a5a-428b-aa4f-4c41b3f6eef0
    ...
    - **TenantId** - Tenant Id where the teams application will be installed.
    ...
    - **Location** - Azure region in which to create the resources. The internal name should be used e.g. eastus. Run the following command in Powershell to list internal names.
      ...
      **PS C:\\&gt;** az account list-locations -o table
...
>**Note** - While updating the user app, script will throw a warning message as below.
...
# Troubleshooting
  ...
  Please see our [Troubleshooting](Troubleshooting-powershell-script) page.